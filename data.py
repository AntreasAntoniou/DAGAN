...
class DAGANDataset(object):
    def __init__(self, batch_size, gan_training_index, reverse_channels, num_of_gpus, gen_batches):
        ...
        self.x_train, self.x_test, self.x_val = self.load_dataset(gan_training_index)
...
class DAGANImbalancedDataset(object):
    def __init__(self, batch_size, gan_training_index, reverse_channels, num_of_gpus, gen_batches):
    ...
    def get_batch(self, dataset_name):
        ...
        for i in range(self.batch_size):
            choose_samples = np.random.choice(len(self.datasets[dataset_name][choose_classes[i]]),
                                              size=2 * self.batch_size,
                                              replace=True)

            choose_samples_a = choose_samples[:self.batch_size]
            choose_samples_b = choose_samples[self.batch_size:]
            current_class_samples = self.datasets[dataset_name][choose_classes[i]]
...
class VGGFaceDAGANDataset(DAGANDataset):
    def __init__(self, batch_size, gan_training_index, reverse_channels, num_of_gpus, gen_batches):
        super(VGGFaceDAGANDataset, self).__init__(batch_size, gan_training_index, reverse_channels, num_of_gpus,
                                                  gen_batches)
